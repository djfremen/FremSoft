...
| 21 | `PDUIoCtl` | MIXED | IoCtl ID dispatch (canonical IDs per `external/pdu_mdi2/pdu_mdi2.c:151-168` + AUTOSAR `external/dpdu-autosar/interface.h`): `PDU_IOCTL_RESET = 1UL` → return 0. `PDU_IOCTL_CLEAR_TX_QUEUE = 2UL`, `PDU_IOCTL_CLEAR_RX_QUEUE = 5UL` → drain queues. `PDU_IOCTL_READ_VBATT = 7UL` → return 12000 mV. `PDU_IOCTL_START_MSG_FILTER = 12UL`, `STOP_MSG_FILTER = 13UL`, `CLEAR_MSG_FILTER = 14UL`. Spec range = 1..18. Most others → return 0 quietly. **Chipsoft vendor IOCTLs** (`0x000C0000`, `0x000C0001`, `0x000C0002` — seen in captures) are outside the spec range; plausibly Chipsoft's private re-numbering of `START/STOP/CLEAR_MSG_FILTER`. For FremSoft MVP handle as opaque pass-through returning 0. *(Earlier draft of this row incorrectly claimed `CLEAR_TX/RX = 0x000C/0x000D` — confusion bleed from the vendor IOCTL family.)* |
